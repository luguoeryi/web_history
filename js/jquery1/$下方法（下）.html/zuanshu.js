...
function Move(obj,json,num,endfn){ //透明度，宽高，定位。。。
...
function opacitys(obj,go,dir,times,endFn){     //透明度封装    //对象  初始0/100 幅度 时间 回调函数
	clearInterval(obj.times);
		...
		if(go>0){
			...
			obj.style.opacity = num/100; //document.all 是IE支持属性 firefox 不支持
...
function getnumarr(str){  //获取字符串中的数字  [12,36,1]
	var arr = [];
	var str1 = '';
...
/*function getByClassName(parent,className){      //范围 类名
